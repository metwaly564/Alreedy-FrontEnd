...
export default function ProductDetails() {
  ...
  const { CartProductsIds, setCartProductsIds, numItems, setnumItems, fetchCartCount, fetchWishlistCount, triggerRefresh } = useContext(CartContext);
    ...
    const fetchProduct = async () => {
      try {
        ...
        const response = await axios.get(
          `https://reedyph.com/api/v1/products/product/${skuId}`
        );
  ...
  async function trackProductView(skuId) {
    try {
      await axios.get(
        `https://reedyph.com/api/v1/products/product-addview/${skuId}`
      );
    } catch (error) {
  ...
  async function addToCart(skuId, quantity = 1) {
    try {
      ...
      try {
        const cartRes = await axios.get("https://reedyph.com/api/v1/carts/cart/", config);
  ...
  const updateCartQuantity = async (newQuantity) => {
    ...
    if (!userToken) {
      ...
      try {
        ...
        await axios.post("https://reedyph.com/api/v1/carts/cart", requestBody, config);
        setQuantity(newQuantity);
        toast.success(isArabic ? "تم تحديث الكمية" : "Quantity updated", { style: { fontFamily: "Alexandria, sans-serif", fontWeight: 300 } });
        if (typeof fetchCartCount === 'function') fetchCartCount();
      } catch (error) {
