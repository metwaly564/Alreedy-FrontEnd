  ...
  async function getData() {
    try {
      ...
      const [categoriesRes, bannersRes] = await Promise.all([
        axios.get("https://reedyph.com/api/v1/categories/main-category"),
        axios.get("https://reedyph.com/api/v1/banners/banner"),
      ]);
        ...
        categoriesRes.data.navBarCategories.map(async (category) => {
          try {
            const productsRes = await axios.get(
              `https://reedyph.com/api/v1/products?categoryId=${category.id}`
            );
            return {
              ...category,
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 overflow-y-hidden">
              ...
              <div className="w-full sm:w-1/2 text-right ">
                ...
                <p className={`text-xs sm:text-sm text-gray-500 mb-2 sm:mb-4 line-clamp-3 ${isArabic ? "text-right" : "text-left"} font-light`}>
                  {isArabic ? selectedProduct.cardDescriptionAr : selectedProduct.cardDescriptionEn}
                </p>
                <div className={`flex flex-col sm:flex-row items-center gap-1 sm:gap-2 ${isArabic ? "justify-end" : "justify-start"} font-light`}>
                  <button
                    className={`${selectedProduct.availableStock <= 0 ? 'bg-gray-600' : 'bg-red-500'} text-white px-3 py-1 rounded-md text-xs sm:text-sm w-full sm:w-auto font-alexandria font-light whitespace-nowrap`}
                    onClick={() => {
