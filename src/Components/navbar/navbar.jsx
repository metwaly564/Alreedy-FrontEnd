...
export default function Navbar() {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          "https://reedyph.com/api/v1/categories"
        );
  ...
  async function handleKeyPress(event) {
    if (event.key === "Enter") {
      ...
      const response = await axios.get(
        `https://reedyph.com/api/v1/products?search=${inputValue}`
      );
  ...
  const toggleMenu = () => {
    ...
    // Reset active tab to 'menu' when opening the menu
    if (!isMenuOpen) {
      setActiveTab("menu");
    }
  };
  ...
  useEffect(() => {
    // No-op, just to trigger re-render
  }, [refreshTrigger]);
          ...
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            ...
            <div className="flex items-center space-x-6 rtl:space-x-reverse">
              <NavLink 
                to="/Offers" 
                className={({ isActive }) => 
                  `uppercase font-medium hover:text-primary-600 transition-colors ${isActive ? 'text-primary-600' : ''}`
                }
              >
                عروض
              </NavLink>
  ...
  {showShopDropdown && (
    <div className={`absolute ${isArabic ? 'right-0' : 'left-0'} mt-2 w-48 bg-white rounded-md shadow-lg z-50 transition-all duration-1000`}>
      ...
      {isMenuOpen && (
        <div
          className="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-50 overflow-hidden"
          onClick={toggleMenu}
