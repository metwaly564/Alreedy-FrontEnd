...
export default function StaticFooter() {
  ...
  const hiddenFooterRoutes = [
    ...
    '/AdminEditBanners', '/ForgetPassVerifyOtp', "/AdminReports","/AdminEditCategory","/AdminEditTags" , "/AdminAddNewTag","/AdminAddNewCat", '/VerifyOtp', '/EditTag'
  ];

  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch('https://reedyph.com/api/v1/categories');
            ...
            <div className="mt-3 text-right">
              {isArabic ? (
                <div className="flex flex-row-reverse w-full">
                  {(() => {
                    // Split categories into 3 columns for Arabic, filling from right to left
                    const colCount = 3;
                    const perCol = Math.ceil(categories.length / colCount);
                    const columns = Array.from({ length: colCount }, (_, i) =>
                      categories.slice(i * perCol, (i + 1) * perCol)
                    );
                    return columns.map((col, idx) => (
                      <ul key={idx} className="flex-1 space-y-1 text-base font-light ">
                        {col.map(category => (
                          <li key={category.id}>
                ...
                <ul className="space-y-2 grid grid-cols-3 items-center justify-center text-[15px] font-light text-left gap-x-5">
      ...
      <div className={`hidden md:flex w-full justify-between gap-8 ${isArabic?"flex-row-reverse":""}`}>
        ...
        <div className={`flex-1 ${isArabic?"text-right":""} `}>
          ...
          <div className={`flex ${isArabic?"text-right justify-end":"flex-row-reverse justify-end"}  items-center mb-2 text-base font-light`}>
            <a href="tel:+201201200016" className={`hover:text-red-400 transition`}>01201200016</a>
            <FaPhone className={`${isArabic?"ml-1":"mr-1"} text-white rotate-90`} size={14} />
          </div>
          ...
          {isArabic ? (
            <div className="flex flex-row-reverse gap-8">
              {(() => {
                ...
                return (
                  <>
                    <ul className="flex-1 space-y-1 text-base font-light">
