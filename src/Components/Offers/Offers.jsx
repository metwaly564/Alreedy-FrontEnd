...
export default function Offers() {
  const { addToCart, addToWishlist, CartProductsIds, setCartProductsIds, numItems, setnumItems } = useContext(CartContext);
  ...
  const [currentSort, setCurrentSort] = useState(isArabic ? "itemRank_desc" : "itemRank_asc");
  ...
  // Fetch offer categories and products
  useEffect(() => {
    const fetchOfferData = async () => {
      try {
        setIsLoading(true);
        // Fetch all categories
        const categoriesResponse = await axios.get("https://reedyph.com/api/v1/categories");
        const rank1Categories = categoriesResponse.data.filter(cat => cat.rank === 1);

        if (rank1Categories.length === 0) {
          setOfferCategories([]);
          setProducts([]);
          setFilteredProducts([]);
          toast.error(isArabic ? "لا توجد عروض متاحة" : "No offers available", {
            style: {
        ...
        setOfferCategories(rank1Categories);

        // Fetch products for each rank 1 category
        const productsPromises = rank1Categories.map(category =>
          axios.get(`https://reedyph.com/api/v1/categories/category/${category.id}`)
        );

        const productsResponses = await Promise.all(productsPromises);
        const allProducts = productsResponses.flatMap(response =>
          response.data.products.filter(product => !product.isDeleted && product.isActive)
        );
        ...
        toast.error(isArabic ? "فشل تحميل العروض" : "Failed to load offers", {
          style: {
            fontFamily: "'Alexandria', sans-serif",
            fontWeight: 300,
            fontSize: "14px"
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        const response = await axios.get('https://reedyph.com/api/v1/tags');
    ...
    <>
      ...
      <div className="mx-auto px-0.5 sm:px-4 relative mt-[14vh] mb-[12vh] bg-gray-100" style={{ fontFamily: "'Alexandria', sans-serif" }}>
        ...
        <div className="text-center mb-8 mt-10 sm:mt-0">
          <h1 className="text-2xl sm:text-3xl font-medium text-gray-800 py-1" style={{ fontFamily: "'Alexandria', sans-serif" }}>
            {isArabic ? offerCategories.map(cat => cat.nameAr).join(' & ') : offerCategories.map(cat => cat.nameEn).join(' & ')}
          </h1>
        </div>
              ...
              >
                {filteredProducts?.length > 0 ? (
                  filteredProducts.map((product) => (
                    <div
                      key={product.skuId}
                      className={`${styles.slideItem} group w-full`}
                    >
                        ...
                        <div className="relative overflow-hidden">
                          ...
                          <div className="absolute top-2 right-3 flex flex-col space-y-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                            <button
                              ...
                              title={isArabic ? "أضف إلى المفضلة" : "Add to Wishlist"}
                            >
        ...
        {isQuickViewOpen && selectedProduct && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[95] p-2 sm:p-4">
            ...
            >
              ...
              {selectedProduct.availableStock <= 0 && (
                <div className="absolute top-0 left-0 bg-gray-600 text-white text-[16px] font-bold px-2 py-0 rounded-md z-10 Alexandria-500 m-4" style={{ fontFamily: "Alexandria", fontWeight: 500 }}>
                  {isArabic ? "غير متوفر" : "Out of Stock"}
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 overflow-y-hidden">
                <div className="w-full sm:w-1/2 flex justify-center relative overflow-y-hidden">
                  <img
                    ...
                    alt={selectedProduct.nameAr || selectedProduct.nameEn}
