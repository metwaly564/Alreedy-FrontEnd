...
export default function AdminEditCateg() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      // Sort categories by rank before setting state
      const sortedCategories = [...data].sort((a, b) => a.rank - b.rank);
  ...
  const handleDelete = async (categoryId) => {
    ...
    try {
      const response = await fetch(
        `https://reedyph.com/api/v1/categories/category/${categoryId}`,
        {
          method: 'DELETE',
          headers: {
  ...
  return (
    <>
      <div className='flex flex-col justify-center font-alexandria font-light mt-16' >
        {error && (
