...
export default function AdminEditPromoCode() {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch('https://reedyph.com/api/v1/promocodes/promocode', {
        method: 'DELETE',
        headers: {
  ...
  const handleEditClick = async (promoCode) => {
    ...
    if (details) {
      setEditModal({
        ...
        categories: details.categories?.map(c => c.id).filter(Boolean) || [],
        users: details.allowedUsers?.map(u => u.id).filter(Boolean) || [],
        cities: details.cities?.map(c => c.id).filter(Boolean) || [],
        zones: details.zones?.map(z => z.id).filter(Boolean) || []
      });
    } else {
      setEditModal({
        ...promoCode,
  ...
  const cleanArray = (arr) => arr ? arr.filter(item => item !== null && item !== undefined) : [];
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        categories: cleanArray(editModal.categories),
        cities: cleanArray(editModal.cities),
        zones: cleanArray(editModal.zones),
        userId: cleanArray(editModal.users)
      };
  ...
  const toggleCategorySelection = (categoryId) => {
    ...
    setEditModal(prev => ({
      ...
      categories: prev.categories.includes(categoryId)
        ? prev.categories.filter(id => id !== categoryId).filter(Boolean)
        : [...prev.categories.filter(Boolean), categoryId]
    }));
  };
  ...
  const toggleZoneSelection = (zoneId) => {
    ...
    setEditModal(prev => ({
      ...
      zones: prev.zones.includes(zoneId)
        ? prev.zones.filter(id => id !== zoneId).filter(Boolean)
        : [...prev.zones.filter(Boolean), zoneId]
    }));
  };
  ...
  const selectAllUsers = () => {
    setEditModal(prev => ({
      ...prev,
      users: users.map(user => user.id).filter(Boolean)
    }));
  };
  ...
  const selectAllCategories = () => {
    setEditModal(prev => ({
      ...prev,
      categories: categories.map(category => category.id).filter(Boolean)
    }));
  };
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        if (results.data.length > 0 && results.data[0].ids) {
          ...
          setEditModal(prev => ({
            ...prev,
            users: [...new Set([...prev.users.filter(Boolean), ...ids])]
          }));
