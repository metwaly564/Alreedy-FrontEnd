...
export default function Wishlist() {
  ...
  const deleteFromWishlist = async (skuId) => {
    try {
      ...
      if (!userToken) {
        toast.error(isArabic ? "يرجى تسجيل الدخول لإدارة قائمة الرغبات" : "Please login to manage your wishlist", { style: { fontFamily: "Alexandria, sans-serif", fontWeight: 300 } });
        return;
      }
      ...
      if (fetchWishlistCount) {
        fetchWishlistCount();
      }
      
      toast.success(isArabic ? "تمت إزالة المنتج من قائمة الرغبات بنجاح!" : "Removed from wishlist successfully!", { style: { fontFamily: "Alexandria, sans-serif", fontWeight: 300 } });
      fetchWishlist();
    } catch (err) {
      console.error("Error deleting from wishlist:", err);
      toast.error(isArabic ? "فشل إزالة المنتج من قائمة الرغبات" : "Failed to remove from wishlist", { style: { fontFamily: "Alexandria, sans-serif", fontWeight: 300 } });
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-64 mt-12 sm:mt-0">
  ...
  if (wishlistItems.length === 0) {
    return (
      <div className="text-center w-full py-10 mt-32" style={{ fontFamily: "'Alexandria', sans-serif" }}>
  ...
  return (
    <>
      ...
      <div className="py-1 px-2 max-w-7xl mx-auto mt-[18vh] bg-gray-100 min-h-screen">
        ...
        <div
          className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-x-1 gap-y-3 sm:gap-x-1 sm:gap-y-3 md:gap-0 pb-28`}
                ...
                <div className="relative overflow-hidden">
                  ...
                  >
                    <img
                      ...
                      className="w-full h-40 sm:h-24 object-contain p-2 transition-transform duration-300 ease-in-out hover:scale-110 overflow-hidden"
