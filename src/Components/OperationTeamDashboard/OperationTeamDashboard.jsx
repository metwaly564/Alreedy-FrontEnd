...
const OperationTeamDashboard = () => {
  ...
  const fetchDeliveryBoys = async () => {
    try {
      const response = await fetch("https://reedyph.com/api/v1/users/all-deliveryBoys", {
        headers: getHeaders()
      });
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Fetch zones
        const zonesResponse = await fetch("https://reedyph.com/api/v1/places/zone", { headers });
        ...
        // Fetch products
        const productsResponse = await fetch("https://reedyph.com/api/v1/products/product", { headers });
  ...
  const updateOrderStatus = async (orderId, status) => {
    try {
      let endpoint = `https://reedyph.com/api/v1/orders/order-${status}/${orderId}`;
      let options = {
        method: 'POST',
        headers: getHeaders()
      };
