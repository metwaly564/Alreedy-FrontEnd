...
const CustomLeftArrow = ({ onClick }) => (
  <button 
    onClick={onClick}
    className="absolute left-2 sm:left-4 bg-black hover:bg-black p-1 sm:p-2 rounded-full text-white z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 top-[55%] sm:top-[45%]"
  >
...
const CustomRightArrow = ({ onClick }) => (
  <button 
    onClick={onClick}
    className="absolute right-2 sm:right-4 bg-black hover:bg-black p-1 sm:p-2 rounded-full text-white z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 top-[55%] sm:top-[45%]"
  >
    <svg xmlns="https://www.w3.org/2000/svg" className="h-4 w-4 sm:h-6 sm:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function MainSlider() {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get('https://reedyph.com/api/v1/banners/banner');
